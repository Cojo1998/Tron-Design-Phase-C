...
//---------------------------------------------------------------------------------------------------Main Loop-----------------------------------------------------------------------------------------------------------
  ...
  { 
      ...
      if (Print == false){
       ...
       /* Serial.print("Coordinates Recieved: X1=");
        Serial.print(BlockX1); //For testing purposes
        Serial.print(" Y1=");
        Serial.print(BlockY1);
        Serial.print(" X2=");
        Serial.print(BlockX2);
        Serial.print(" Y2=");
        Serial.println(BlockY2);
        */
      ...
      if (Start1 == "s")    //Enters if statement once an "s" is received.
      {
        ...
        while (i<5)
        {
          ...
          if ((check < 50) && grabFlag == 0 && i<4)
          {
            if(grabReset == 0)
            {
              //preform pick up and drop off
              DesiredSpeedR = Speed;
              DesiredSpeedL = Speed;
              delay(1000);     //Hudson Edit remove if faulty
              DesiredSpeedR=0;
              DesiredSpeedL=0;
...
void TimerInterrupt() // -------------------Timer Interrupt-------------------------------------------------------------------------------------------------------------------------------------------------
{
  ...
  volatile float ErrorR,ErrorL, ChangeInErrorR,ChangeInErrorL;
    ...
    MotorVoltageR = -MotorVoltageR + 9;
    MotorVoltageL = -MotorVoltageL + 5;
  }

   //Update Motor voltages
   //Serial.println(LeftAdjustmentPID);
   //Serial.println(MotorVoltageL);
   motor.control(-MotorVoltageR + 10, MotorVoltageL - 5);      // Right Motor is physically reveresed normally +9 right and -5 left       when dead ish  +9   , -7
...
float FindSpeedL(float DS)    
{
 ...
 void angledRightTurn()
  {
    int j = 500*(angleDiff);    //Calculates the appropriate delay which will result in the correct turning angle being achieved based on testing and calibration. 500 half life
  ...
  void angledLeftTurn()
  {
    int k = 525*(angleDiff);    //Calculates the appropriate delay which will result in the correct turning angle being achieved based on testing and calibration. 300 half life
  ...
  void GripperOpen(){
   ...
   //newX = Serial.parseFloat();
   //newY = Serial.parseFloat();
   // currentAngle=atan2((newY - startY),(newX - startX));
